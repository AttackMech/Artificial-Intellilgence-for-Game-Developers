...
import spacepizza.steering.LookWhereGoing;
import spacepizza.steering.Seek;
import spacepizza.steering.SteeringOutput;
	...
	public void update(long time, Kinematic player) {
		...
		for (int i = 0; i < thrownWeapons.size(); i++) {
			...
			if (wpn.getType() == Weapon.CUTTER && System.nanoTime() / 1000000 - wpn.getBeginTime() / 1000000 > 5000) {
				...
				wpn.setTarget(player.position.x, player.position.y);
			} else {
